...
public class MeiTuanFacadeService {
    ...
    public String closeShop(String code)
    {
        String json = "";
        try {
             json = mtApiService.poiClose(code);
        }catch (MeiTuanException ex){
     ...
     * @param orderId - 订单id
     */
    public String getCancelOrder(Long orderId,String reason,String reasonCode)
    {
        String json = "";
        try {
             json = mtApiService.getCancelOrder(orderId,reason,reasonCode);
        }catch (MeiTuanException  ex) {
